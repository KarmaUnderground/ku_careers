...
function transform_job_2_esx_jobs(jobs)
    ...
    for cycle_name, job_data in pairs(jobs) do
        ...
        for item, values in pairs(job_data.items) do
            trans_job.Zones[item] = values.position
            trans_job.Zones[item].Item = values

            trans_job.Zones[item].Item.position = nil

            if item == name then
                trans_job.Delivery = job_data.Vehicles[1].vendor
